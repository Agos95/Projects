...
def S_R_inf_surfaces(mu,beta,time,S,I,R,number_of_nodes,filename=None):
    """
    ...
    ax.set_xlabel("$\\beta$ $[day^{-1}]$ ",fontsize=14)
    ax.set_ylabel("$\\mu$ $[day^{-1}]$",fontsize=14)
    ax.set_zlabel("$S_\\infty$",fontsize=14)
...
def beta_mu_heatmap(beta, mu, x, N, fname=None, comparison=False):
    """
    ...
    if np.all(np.array(x_inf)>=0) and not comparison:
        vmax = 1
        vmin = 0
        center = None
        label = "$s_\\infty$"
        ...
        vmin = None
        center = 0
        label = "$(s_\\infty - s_\\infty^{Removed}) / s_\\infty$"
...
def tagli(mu,beta,cosa_tagliare,number_of_nodes,values,title,filename1=None,filename2=None):
    """
    ...
    for v in values:
        ...
        """plt.plot(Y[sel], np.divide(cosa_tagliare[sel], number_of_nodes),label="$\\beta$ = {:.3f}".format(v))
    ...
    for v in values:
        ...
        plt.plot(X[sel], np.divide(cosa_tagliare[sel], number_of_nodes), label="$\\mu$"+" = {:.3f}".format(v))
